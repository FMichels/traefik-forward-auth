...
func ValidateCookie(r *http.Request, c *http.Cookie) (string, string, string, error) {
	...
	expectedSignature := cookieSignature(r, parts[2], parts[3], parts[4], parts[1])
...
func MakeCookie(r *http.Request, email string, user string, fullName string) *http.Cookie {
	expires := cookieExpiry()
	mac := cookieSignature(r, email, user, fullName, fmt.Sprintf("%d", expires.Unix()))
	value := fmt.Sprintf("%s|%d|%s|%s|%s", mac, expires.Unix(), email, user, fullName)
...
// Create cookie hmac
func cookieSignature(r *http.Request, email, user, fullName, expires string) string {
